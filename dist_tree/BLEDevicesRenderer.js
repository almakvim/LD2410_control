...
function itemDone(element) {
  console.log('itemDone', element.id)
...
window.api.receive('BLEDeviceList', (data) => {
  ...
  for (let i = 0; i < diff; i++) {
    ...
    console.log('BLEDevicesListarray', BLEDevicesListarray[index])
    item.id = BLEDevicesListarray[index].deviceId

    // Set its contents:
    item.appendChild(
      document.createTextNode(
        BLEDevicesListarray[index].deviceName +
          ' (' +
          BLEDevicesListarray[index].deviceId +
          ')'
      )
    )
...
window.api.receive('BLEScannElement', (data) => {
  console.log('BLEScannElement', data)
})
